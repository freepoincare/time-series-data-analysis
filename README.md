...
```mermaid
flowchart TD
    A["1. 원시 데이터 수집 & 정제<br/>(KMA ASOS #108, 42,236건)"] --> B["2. 기술 통계 및 시계열 집계<br/>(연별/계절별/월별 기온 지표 산출)"]
    B --> C["3. 장기 추세 통계 분석<br/>(10년 이동평균, OLS 회귀, Mann-Kendall)"]
    C --> D["4. 계절 경계 확장/축소 분석<br/>(5월/9월 초·늦여름화, 3월/11월 겨울 축소)"]
    D --> E["5. 시계열 예측 모델링<br/>(ARIMA(0,1,1) vs 선형 회귀 5개년 예측)"]
    E --> F["6. 대시보드 서비스화 & 리포트<br/>(Streamlit 웹 대시보드 & REPORT.md)"]
```
...
5. **여름철 기온 예측 (`5_summer_forecasting.ipynb`):**
